...
const char WM_HTTP_HEAD[] PROGMEM            = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"UTF-8\" name=\"viewport\" content=\"width=device-width, initial-scale=1, user-scalable=no\"/><title>{v}</title>";
...
const char WM_HTTP_PORTAL_OPTIONS[] PROGMEM  = "<form action=\"/wifi\" method=\"get\"><button>Configure</button></form><br/><form action=\"/i\" method=\"get\"><button>Hardware Info</button></form><br/><form action=\"/r\" method=\"post\"><button>Reset</button></form>";
const char WM_HTTP_ITEM[] PROGMEM            = "<div><a href='#p' onclick='c(this)'>{v}</a>&nbsp;<span class='q {i}'>{r}</span></div>";
const char WM_HTTP_SAVED_APS[] PROGMEM       = "<p><div>Saved Networks:</div>";
const char WM_HTTP_FORM_START[] PROGMEM      = "<form method='get' action='wifisave'><input id='s' name='s' length=32 placeholder='SSID'><br/><input id='p' name='p' length=64 type='password' placeholder='password'><br/><br/>";
const char WM_HTTP_FORM_PARAM[] PROGMEM      = "<br/><div>{p}:</div><input id='{i}' name='{n}' maxlength={l} value='{v}' {c}>";
...
const char WM_HTTP_SAVED[] PROGMEM           = "<div>Settings Saved<br />Trying to connect to network.<br />If it fails reconnect to AP to try again</div>";
